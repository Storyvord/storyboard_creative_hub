    ...
    <div className={clsx(
      ...
      <div className="p-4" onDragEnd={handleDragEnd}>
        {loadingShots ? (
          ...
          <div {...(shotsAreaTourId ? { "data-tour": shotsAreaTourId } : {})} className="flex items-center gap-0 overflow-x-auto pb-2 scroll-smooth"
...
export default function StoryboardPage() {
  ...
  return (
    <div className="flex h-full bg-[#0a0a0a] overflow-hidden relative">
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="h-14 border-b border-[#1a1a1a] bg-[#0d0d0d] flex items-center justify-between px-5 z-10">
          ...
          <div className="flex items-center gap-2">
            <TourTriggerButton onClick={() => setIsTourVisible(true)} />
              ...
              <>
                {scenes.map((scene, sceneIndex) => (
                  <div id={`scene-${scene.id}`} key={scene.id} className="scroll-mt-6"
                    {...(sceneIndex === 0 ? { "data-tour": "scene-row" } : {})}
                  >
      ...
      {isTourVisible && (
        <StoryboardTour onComplete={() => setIsTourVisible(false)} />
      )}
    </div>
  );
}
