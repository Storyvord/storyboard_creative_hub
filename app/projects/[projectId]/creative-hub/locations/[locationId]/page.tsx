...
export default function LocationDetailPage() {
    ...
    const handleAttachRefs = async (files: FileList | File[]) => {
        if (!location?.script) {
            toast.error("Open a location with a script first.");
            return;
        }
        ...
        for (const file of valid) {
            try {
                const ref = await uploadCreativeSpaceReference(location.script, file);
    ...
    const handleModelConfirm = async (model: string, provider: string) => {
        ...
        try {
            ...
            const result = await generateLocationImage(locationId, model, provider, refIds);
                ...
                <div className="space-y-4 order-2 lg:order-1">
                    ...
                    <div className="hidden md:flex items-center gap-2 flex-wrap">
                        <button
                            ...
                            disabled={!location?.script || generating}
