# Creative Hub — Tutorial Video Series Script
...
|---|---|---:|---:|
| 0 | Creative Hub Overview | 216 | ~93s |
| 1 | Script | 417 | ~179s |
| 2 | Scenes | 376 | ~161s |
| 3 | Shots | 373 | ~160s |
| 4 | Storyboarding | 423 | ~181s |
| 5 | Characters | 428 | ~183s |
| 6 | Locations | 352 | ~151s |
| 7 | Creative Space — Image Models | 428 | ~183s |
| 8 | Consistency in Images | 402 | ~172s |
| **Total** | | **3415** | **~24m 24s** |

---

## Chapter 0 — Creative Hub Overview
**Standalone video runtime:** ~93s  **VO word count:** 216  **Difficulty:** Beginner
...
**Getting in.** Open any Storyvord project and look at the left sidebar. The Creative Hub menu lists every page in the order you'll touch them.
...
- **Creative Space** is the freeform image studio for everything that doesn't fit a shot row.
...
## Chapter 1 — Script
**Standalone video runtime:** ~179s  **VO word count:** 417  **Difficulty:** Beginner
...
3. **Confirm the script.** Hit Confirm Script. If you edited the text, the parser re-runs on your edits; if you didn't, it accepts the existing parse. Now Scenes, Characters, Locations, and Storyboarding all unlock.
...
## Chapter 2 — Scenes
**Standalone video runtime:** ~161s  **VO word count:** 376  **Difficulty:** Beginner
...
### What this feature does (10-20s)
> Once the script is confirmed, every scene becomes a card on a vertical strip. You can see at a glance what's interior versus exterior, where it shoots, who's in it, and which scenes the Risk Analyzer has flagged. It's the bridge between the literary script and the physical production.

### How to use it — walkthrough

**Getting in.** From the Creative Hub sidebar, open Scenes. You'll see one card per scene, in order.

**Reading a scene card.** Each card has a square slug on the left — "SC 03" — then the scene name, the INT or EXT chip, the environment, a two-line description, and a location pin. If the Risk Analyzer has scored this scene, you'll also see a risk pill — for example, "HIGH · 3" — on the right.
...
## Chapter 3 — Shots
**Standalone video runtime:** ~160s  **VO word count:** 373  **Difficulty:** Intermediate

### Cold-open orientation (5-10s)
> Shots in Creative Hub don't have their own top-level page. They live inside Scene Detail and inside the Storyboard. This video covers what a shot row contains, how to edit and reorder shots, and how to tag characters into a shot so the AI uses the right faces.
...
**Where to find shots.** Open a scene from the Scenes page and scroll to the bottom — you'll see the shot grid with a Generate Shots button. Or go to Storyboarding, where every scene is a horizontal filmstrip of shots.
...
**Generating shots.** Hit Generate Shots on a Scene Detail page, or select scenes in the Storyboard and use Bulk Shots. The AI reads the scene description and returns an ordered sequence of shots with descriptions, types, and angles already filled in. A per-scene loader runs while the task is in flight.
...
**Standalone video runtime:** ~181s  **VO word count:** 423  **Difficulty:** Intermediate
...
**Standalone video runtime:** ~183s  **VO word count:** 428  **Difficulty:** Intermediate
...
**The Scene Look Editor.** Three panes.
- **Left:** the canonical Character Ref portrait above, the Scene Look canvas below. The pair is the consistency story made visible — the face on top, the scene-specific variant below.
- **Center:** the wardrobe picker. Selected items sit in the Assigned Costume rack at the top; below them, tabbed categories — Head, Face, Torso, Legs, Feet, Hands, Full Body, Accessories — let you pick from your wardrobe library.
- **Right:** Style Direction for lighting, mood, era, and color palette, and Continuity Notes for injuries, aging, makeup FX, blood, costume damage, and props.
...
## Chapter 6 — Locations
**Standalone video runtime:** ~151s  **VO word count:** 352  **Difficulty:** Beginner
...
**The parameter bar.** Above the send button:
- **Aspect ratio** — the same eight options as the storyboard.
- **Camera angle** and **shot type** — modal pickers with the same options as a shot card.
- **Model** — a dropdown of every image model available, each with a credits-per-image cost shown next to the name. Some models have **quality** and **resolution** variants; if so, those dropdowns appear next to the model.
...
**Standalone video runtime:** ~172s  **VO word count:** 402  **Difficulty:** Advanced

### Cold-open orientation (5-10s)
> AI image generators drift. The same character will look different in every shot unless you tell the model what to lock. This video covers the four mechanisms Creative Hub uses to fight drift — and how to use each one deliberately.
...
**Lever one: active reference per subject.** Every character, every location, every scene character has one image marked as the active reference. Open the character detail page or the location detail page and you'll see a Previz History strip below the hero. Any image in that history can be promoted to active — click it, or open the Compare modal to look at two side by side, and you're choosing which look becomes canonical. Every downstream generation that mentions that subject will use the newly active image.

**Lever two: `@` and `#` mentions in prompts.** Inside a shot description in the Storyboard, or inside Creative Space, typing `@CharacterName` resolves to that character's active reference. Typing `#LocationName` pulls in the location's establishing image. You're not just tagging metadata — you're telling the image model which faces and which places to use.

**Lever three: user reference uploads.** Anywhere you see an Add Reference control — character detail, location detail, Creative Space — you can drop in supplementary images. They become `$1`, `$2`, `$3`, and the next generation passes them all through as reference inputs. Useful for face refs, mood boards, costume swatches. Models that charge for input images will show the per-reference cost in the model selector.

**Lever four: style and aspect locking.** The Storyboarding toolbar carries a project-wide style — Sketch, Storyboard, HD, Anime — and a project-wide aspect ratio. Both write back to the script so every shot generation defaults to them. A single scene can override the style — say, a flashback in Sketch while the rest is HD — and the X next to the override resets it to inherit the project default.

**The anchor concept.** Promoting an image to active reference is the most important action in Creative Hub. Every other consistency mechanism feeds from that one decision.

### Director notes
- The Compare modal is the visual hero — show two images side by side and the promote-to-active click.
- Slow down on the anchor concept beat. Hold two seconds after the line lands.
...
## Change log narrative_v3 → explainer_v1

- Killed the "Alex / three people / one film / drawn" narrative spine entirely; no fictional characters, no thesis arc, no callback lines.
- Reframed every chapter from brand-film beat (tension → reveal → killer detail → handoff) to tutorial register (orient → what it does → how to use it → outputs → cross-links).
- Each chapter now opens with a one-sentence cold-open orientation so a fresh viewer landing from search isn't lost.
- Removed the silent slideshow climax and held-visual beats; tutorial videos run continuous narration with explicit step ordering.
- Targeted 90-180s per feature chapter (up from ~60-90s brand-film cap) to make room for concrete how-to steps.
- Kept Linear-style production rules: filmmaker vocabulary, narrate intent over UI labels, no SaaS jargon, declarative voice.
- Added an explicit `[DEMO]` honesty pass on Locations so viewers don't expect weather/parking/contacts data that isn't wired up.
- Reframed Chapter 8 (Consistency) as four named "levers" with `setActiveSubjectPreviz` as the anchor concept, presented as a feature explainer rather than a story payoff.
- Removed all "three people, one film" closing matter; replaced outro with practical cross-reference / pinned-comment language for a YouTube series.
